<template>
  <div class="search-container">
    ...
    />
    <button @click="searchCity" class="search-container__button">Search</button>
    <div v-if="showDropdown" class="search-container__dropdown">
      <div
        v-for="city in suggestion"
        :key="city.name"
        class="search-container__dropdown-item"
...
const { cityData, sevenDayForecastData, fetchCityData, fetchSevenDayForecast } = useCity()
...
const searchCity = async () => {
  ...
  if (cityData.value && cityData.value.length > 0) {
    ...
    await fetchSevenDayForecast(cityData.value[0].lon, cityData.value[0].lat)
...
.search-container__dropdown-item {
  padding: 0.5rem;
  color: black;
  cursor: pointer;
  text-align: start;
...
.search-container__dropdown-item:hover {
